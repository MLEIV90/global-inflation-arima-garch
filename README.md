...
**Modelado**: `auto_arima` (pmdarima) por serie, `d∈{0,1,2}`, `p,q∈{0..5}`, selección por AICc; GARCH(1,1) (`arch`) sobre los residuos del ARIMA, con `rescale=True` para manejar hiperinflación numéricamente. La evaluación es **walk-forward**: ventana de 24 meses, forecast a 1 paso con origen móvil, re-estimando el orden completo cada 6 pasos. Contra esto se compara con **dos baselines**: el ingenuo (random walk: "el próximo valor es el último observado") y el de **Atkeson & Ohanian (2001)** (media móvil de los últimos 12 meses) — la prueba de honestidad de si el modelo aporta algo sobre lo trivial, contra dos referencias distintas en vez de una sola.

**Hallazgo del segundo baseline (Atkeson-Ohanian)**: ARIMA le gana a AO en **99.1% de las series (634/640)**, muy por encima del 63.1% contra random walk. No confirma la hipótesis inicial de que AO sería el benchmark más exigente de los dos — acá resultó ser el más débil, porque la ventana de evaluación (últimos 24 meses de cada serie, mayormente hasta 2025-03) coincide con la desinflación post-pandemia: un cambio de tendencia marcado que penaliza a una media móvil de 12 meses (reacciona con rezago) mucho más que a un random walk (se ajusta cada mes). Detalle en `reports/remediacion_R1_atkeson_ohanian.md`.
...
- **El GARCH se ajusta sobre residuos ARIMA que en su mayoría no son ruido blanco** (auditoría integral, hallazgo C.2 — remediado en `reports/remediacion_R2_residuos_no_blancos.md`): **82.9% de los ajustes GARCH (519/626)** corren sobre residuos con Ljung-Box p<0.05. Es parcialmente un artefacto del tamaño de muestra — las series marcadas son sistemáticamente más largas (mediana 290 vs. 170 meses, Mann-Whitney p=4.7×10⁻²⁴), y a mayor n el test detecta autocorrelación cada vez más chica como "significativa" — pero eso no descarta que parte sea especificación real insuficiente. El flag `residuos_no_blancos` en `resultados_enriquecidos.parquet` marca los casos afectados. El hallazgo central del proyecto (gradiente por ingreso) usa el RMSE walk-forward y no se ve afectado; el hallazgo de Análisis C sobre ausencia de patrón en persistencia GARCH sí debe leerse con esta salvedad.
- **El baseline de comparación importa, y el resultado depende de cuál se use** (auditoría integral, hallazgo C.5 — remediado en `reports/remediacion_R1_atkeson_ohanian.md`): además del random walk, se agregó el benchmark de Atkeson & Ohanian (2001) — media móvil de 12 meses. Contra AO, ARIMA gana en **99.1% de las series (634/640)**, muy por encima del 63.1% contra random walk — resultado opuesto a la hipótesis original de que AO sería un benchmark más exigente. La razón: la ventana de evaluación (últimos 24 meses de cada serie, mayormente hasta 2025-03) coincide con la desinflación post-pandemia, un cambio de tendencia marcado ante el cual una media móvil de 12 meses reacciona con rezago severo, mientras el random walk se ajusta cada mes. AO es un benchmark difícil solo en regímenes estables sin tendencia — no en esta ventana.
- **~27 series con cobertura sustancial (algunas con 600+ meses de historia) quedan fuera del panel modelado por un único hueco interno** (auditoría integral, hallazgos A.3/B.1 — documentado, no remediado en el código): el criterio `apto_arima`/`apto_garch` exige **cero huecos internos absolutos**, sin excepción de tamaño. Series como Irlanda (fcpi, 616 meses válidos) o Tailandia (ppi, 610 meses) quedan completamente excluidas por un solo mes faltante. Ninguno de los análisis reportados está invalidado por esto — todos declaran explícitamente que trabajan sobre el universo apto — pero el panel modelado es más chico de lo que la cobertura de datos disponible permitiría con una regla menos estricta (ej. tolerar huecos de 1 mes interpolables). Queda como mejora futura, no implementada en esta remediación.
- **3 pares de países tienen la serie `hcpi` byte-idéntica en la fuente del Banco Mundial** (auditoría integral, hallazgo A.7 — `reports/diagnostico_series_duplicadas.md`; documentado, remediación diferida por costo-beneficio): **FIN/GAB, GBR/USA y CZE/DJI** comparten, literalmente, la misma fila de 663 (o 411) meses en `hcpi_m` — imposible que sea un dato real. Confirmado que es un **error de origen del Excel del Banco Mundial, no del ETL de este proyecto** (la búsqueda corrió directo sobre `data/raw/Inflation-data.xlsx`, antes de cualquier transformación propia). Un análisis forense contra la hoja anual oficial (`hcpi_a`, independiente del mensual) de cada país da veredicto razonablemente claro para 2 de los 3 pares: **Gabón y Estados Unidos son las copias** (su propia tasa anual oficial se aparta 0.37 y 1.34 puntos porcentuales, respectivamente, de lo que implica el mensual compartido) — **Finlandia y Reino Unido son los originales** (su tasa anual coincide a 0.03-0.05 puntos). El tercer par, **Chequia/Yibuti, queda indeterminado** (distancias casi idénticas). **Impacto medido, no supuesto**: se re-corrió el test central del proyecto (Kruskal-Wallis del gradiente de ingreso) excluyendo los 6 países afectados — el gradiente se mantiene igual de significativo, de hecho levemente más fuerte (**p=1.14×10⁻⁶ sin estas series, vs. p=5.6×10⁻⁷ con ellas**, n=168 vs. 174) — la conclusión central de este proyecto **no se ve afectada**. Se decidió documentar este hallazgo en vez de excluir las series o re-correr el modelado, precisamente porque el impacto medido es nulo: el costo de una remediación de código no se justifica frente a un beneficio ya confirmado como cero sobre las conclusiones publicadas. Vale reconocer honestamente que la auditoría formal (hallazgos A.1-A.6) **no detectó esto** — solo buscaba `Country Code` duplicado *dentro* de una misma hoja (mismo país, dos filas), no series idénticas *entre* países distintos; apareció recién durante la remediación R1 (verificación del benchmark Atkeson-Ohanian), al notar coincidencias exactas entre países al calcular RMSE.
...
- **Reportar y resolver el error de datos de origen (hallazgo A.7)**: reportar al Banco Mundial las 3 series `hcpi` duplicadas entre países (ver "Limitaciones honestas"), e investigar el par indeterminado (Chequia/Yibuti) contra una fuente externa independiente (FRED, FMI/IFS) para determinar cuál de los dos tiene la serie real y poder, recién entonces, corregir o excluir con confianza.
...
| `src/13_benchmark_atkeson_ohanian.py` | Remediación C.5: agrega benchmark Atkeson-Ohanian (media móvil 12m) como segundo baseline |
| `src/14_flag_residuos_no_blancos.py` | Remediación C.2: agrega flag `residuos_no_blancos` (Ljung-Box p<0.05) para trazabilidad del GARCH |
...
**Remediación E.4/R5 (auditoría integral)**: estos dos scripts generaban `data/processed/calidad_series.parquet` y `resultados_modelos.parquet` respectivamente — artefactos huérfanos que no cubría `run_pipeline.py` ni usaba ningún otro script/notebook del repo. Se decidió **no** agregarlos al pipeline (harían correr de nuevo un modelado ya superseded por `06b`, sin aportar nada al resultado final) y en cambio **eliminar los dos `.parquet` del repo** — si se necesita reproducirlos por curiosidad histórica, los scripts siguen acá y se pueden correr manualmente; lo que se evita es que queden versionados como si fueran parte del resultado reproducible.
