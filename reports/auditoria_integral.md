...
**Severidad:** 🟠 **SIGNIFICATIVO** (no 🔴 crítico — ver justificación abajo) → **CERRADO: documentado, remediación diferida por costo-beneficio (impacto nulo medido)** (2026-08)
...
**Remediación sugerida (evaluada, no implementada):**
...
**Decisión de cierre (2026-08)**: se evaluó explícitamente la opción 2 (excluir las 6 series y re-correr Análisis A) contra dejar el hallazgo documentado sin tocar el panel modelado, y se optó por **documentar, no remediar con exclusión**. Razón: el impacto ya está medido, no es una suposición — el gradiente de ingreso se sostiene prácticamente igual con o sin estas series (p=5.6×10⁻⁷ con, p=1.14×10⁻⁶ sin), así que una exclusión y re-modelado no cambiaría ninguna conclusión publicada, solo el n de una tabla. El costo (re-correr el pipeline de modelado, actualizar 5+ reportes derivados, mantener una excepción de por vida hasta resolver la fuente) no se justifica frente a un beneficio ya confirmado como nulo sobre las conclusiones. Se documentó de forma prominente en `README.md` ("Limitaciones honestas") y en `notebooks/informe_completo.ipynb`, y se dejó como recomendación de trabajo futuro reportar el error al Banco Mundial e investigar el par indeterminado (CZE/DJI) contra una fuente externa (FRED/FMI). Este hallazgo permanece 🟠 SIGNIFICATIVO en su clasificación de severidad — "cerrado" se refiere a la decisión de remediación, no a una reclasificación de la severidad del problema de datos en sí.
...
| A.7 | Datos | 3 pares país-país con serie `hcpi` byte-idéntica en la fuente (hallazgo post-cierre) | 🟠 Significativo — cerrado: documentado, remediación diferida (impacto nulo medido) | Confirmado exhaustivo (5 hojas, Excel crudo); gradiente sobrevive exclusión (p=1.14e-06); documentado en README y notebook |
...
**Totales por severidad (Fases A-G, auditoría completa, estado post-remediación 2026-08):** 🟠 Significativo: 3 (A.3, B.1 —mismo problema raíz, sin remediar—; C.2 con trazabilidad remediada pero filtro pendiente; A.7, hallazgo nuevo post-cierre, **cerrado como documentado/remediación diferida**) · 🟡 Menor: 1 (E.4, remediado parcialmente — artefactos legacy resueltos, notebook pendiente) · 🔵 Observación: 8 (A.5, B.5, C.1, C.3, C.4, D.1, D.4, G.2) · ✅ Remediado: 2 (C.5, F.3) · Sin hallazgo (fortaleza/confirmado/no-confirmado): 15 (A.1, A.2, B.2, B.3, D.2, D.3, D.5, E.1, E.2, E.3, F.1, F.2, F.4, G.1, G.3).
...
**Recomendación:** el proyecto puede presentarse como está, citando esta auditoría como evidencia de rigor y auto-crítica documentada. Del lote de remediación de 2026-08, C.5 y F.3 quedan totalmente resueltos; C.2 tiene trazabilidad pero no el filtro de fondo; A.3/B.1 sigue sin remediar en el código; **A.7 queda cerrado por decisión explícita** — documentado de forma prominente, sin exclusión de código, por costo-beneficio (impacto ya medido como nulo). Se recomienda que una futura fase correctiva aborde específicamente **A.3/B.1** (decidir si relajar el criterio de cero-huecos) y complete **C.2** (filtrar o repetir el análisis de persistencia restringido a residuos limpios) antes de cualquier extensión del alcance del proyecto (más países, más indicadores, nuevos modelos), para no propagar el mismo criterio de exclusión de huecos o la misma falta de filtro pre-GARCH a un panel más grande. Para **A.7**, la acción pendiente no es de código sino externa: reportar el error al Banco Mundial e investigar el par indeterminado (CZE/DJI) contra una fuente independiente (FRED/FMI) antes de considerar cualquier corrección de datos.
